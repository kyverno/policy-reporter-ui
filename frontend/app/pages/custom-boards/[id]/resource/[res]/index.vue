<template>
  <v-container fluid class="py-4 px-4 main-height" :key="id" v-if="data">
    <app-row>
      <v-card elevation="2" rounded>
        ...
        <v-card-text>
          <graph-resource-result-counts v-if="type !== 'severity'" :data="data.results" />
          <graph-resource-severity-counts v-else :data="data.severityResults" />
        </v-card-text>
      </v-card>
    </app-row>
    ...
    <resource-scroller :list="data.sources">
      <template #default="{ item }">
        <custom-board-category-tables :id="cb" :source="item" :resource="data.resource" />
      </template>
    </resource-scroller>
  </v-container>
</template>
...
const cb = computed(() => route.params.id as string)
const id = computed(() => route.params.res as string)
...
const filter = computed(() => {
  ...
  if (route.query.categories) {
    if (typeof route.query.categories === 'object') { f.categories = route.query.categories as string[] }
    if (typeof route.query.categories === 'string') { f.categories = [route.query.categories] }
  }

  if (route.query.sources) {
    if (typeof route.query.sources === 'object') { f.sources = route.query.sources as string[] }
...
const { data, refresh } = useAPI((api) => api.customBoardResource(cb.value, id.value, filter.value));
