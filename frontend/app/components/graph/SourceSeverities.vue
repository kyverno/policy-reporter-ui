    ...
    <v-col cols="12" md="8">
      <v-card style="height: 100%;">
        <v-card-text style="height: 100%" v-if="showExpanded">
          <GraphCountPerNamespace :data="charts.complete" />
        </v-card-text>
        <v-card-text style="height: 100%" v-if="showExpanded && charts.preview">
          <GraphCountPerNamespace :data="charts.preview" />
        </v-card-text>
  ...
  <template v-if="data.clusterScope">
    <app-row>
      <v-card :title="`${capilize(source)} Cluster Scoped Results`">
        <GraphClusterSeverityCounts :data="clusterCharts" />
      </v-card>
    </app-row>
  </template>
</template>
...
import type {Dashboard, Findings, NamespaceCharts, ClusterCharts} from "~/types/core";

const props = defineProps<{ data: Dashboard; source: string; category?: string; }>();
const findings = ref(props.data.charts.findings as Findings)
const charts = ref(props.data.charts.namespaceScope[props.source] as NamespaceCharts)
const clusterCharts = ref(props.data.charts.clusterScope[props.source] as ClusterCharts)
