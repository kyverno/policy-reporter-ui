    ...
    <v-row v-if="show">
      <v-col v-if="policy.engine" cols="12" lg="5">
        <v-card title="Engine Information">
          ...
          <tbody>
            <tr class="top-border">
              ...
              <td>{{ policy.engine.name }}</td>
            </tr>
            <tr class="top-border" v-if="policy.engine.version">
              <td class="font-weight-bold">Min. Version</td>
              <td>{{ policy.engine.version }}</td>
            </tr>
            <tr v-if="(policy.engine.subjects || []).length">
              <td class="font-weight-bold">Subjects</td>
              <td>{{ policy.engine.subjects.join(', ') }}</td>
            </tr>
            </tbody>
          </v-table>
        </v-card>
        <v-card v-if="policy.details && policy.details.length" class="mt-4" title="Details">
          ...
          <v-table>
          <tbody>
            <tr class="top-border" v-for="item in policy.details" :key="item.value">
              <td class="font-weight-bold" v-if="item.title">{{ item.title }}</td>
              <td>{{ item.value }}</td>
            </tr>
          </tbody>
          </v-table>
...
import { type PolicyDetails } from "~/types/core";
